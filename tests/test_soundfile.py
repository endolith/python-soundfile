...
def test_blocks_full_last_block(file_stereo_r):
    ...
    assert_equal_list_of_arrays(blocks, [data_stereo[:2], data_stereo[2:4]])


def test_blocks_partial_last_block(file_stereo_r):
    blocks = list(sf.blocks(file_stereo_r, blocksize=3))
    assert_equal_list_of_arrays(blocks, [data_stereo[:3], data_stereo[3:4]])
...
def test_blocks_fill_last_block(file_stereo_r):
    ...
    assert_equal_list_of_arrays(blocks, [data_stereo[:3], last_block])


def test_blocks_with_overlap(file_stereo_r):
    blocks = list(sf.blocks(file_stereo_r, blocksize=3, overlap=2))
    assert_equal_list_of_arrays(blocks, [data_stereo[:3], data_stereo[1:4]])
...
def test_blocks_with_stop(file_stereo_r):
    ...
    assert_equal_list_of_arrays(blocks, [data_stereo[:2]])
...
def test_blocks_with_frames(file_stereo_r):
    ...
    assert_equal_list_of_arrays(blocks, [data_stereo[:2], data_stereo[2:3]])
...
def test_blocks_with_frames_and_fill_value(file_stereo_r):
    ...
    assert_equal_list_of_arrays(blocks, [data_stereo[:2], last_block])
...
def test_blocks_inplace_modification(file_stereo_r):
    ...
    expected_blocks = [data_stereo[:3], data_stereo[2:5]]
...
def test_libsndfile_error_with_prefix():
    ...
    assert isinstance(err, sf.SoundFileError)
    assert err.error_string # cannot assume exact message generated by libsndfile
    assert str(err) == f"XX {err.error_string}"
